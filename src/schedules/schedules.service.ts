...
export class SchedulesService {
  ...
  async create(createScheduleDto: CreateScheduleDto): Promise<Schedule> {
    ...
    if (createScheduleDto.event_id) {
      const event = await this.eventsService.queryCompetitionDetails(
        createScheduleDto.event_id,
      );
      if (!event) {
        throw new NotFoundException(
          `Event with id ${createScheduleDto.event_id} not found`,
        );
      }
    }
  ...
  async update(
    ...
    if (updateScheduleDto.event_id) {
      const event = await this.eventsService.queryCompetitionDetails(
        updateScheduleDto.event_id,
      );
      if (!event) {
        throw new NotFoundException(
          `Event with id ${updateScheduleDto.event_id} not found`,
        );
      }
    }
    ...
    if (updateScheduleDto.is_published !== undefined)
      updatedSchedule.is_published = updateScheduleDto.is_published;
    if (updateScheduleDto.event_id) {
    ...
    if (updateScheduleDto.items && updateScheduleDto.items.length > 0) {
      // 删除原有的items
      await this.scheduleItemRepository.delete({
        schedule: { schedule_id: id } as Schedule,
      });
